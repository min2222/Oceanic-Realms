...
public class GiantKelpFeature extends Feature<NoneFeatureConfiguration>
{
	public GiantKelpFeature(Codec<NoneFeatureConfiguration> p_66219_) 
	{
		super(p_66219_);
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> p_159956_)
	{
		...
		{
			BlockState blockstate = OceanicBlocks.GIANT_KELP.get().defaultBlockState();
			BlockState blockstate1 = OceanicBlocks.GIANT_KELP_PLANT.get().defaultBlockState();
			...
			for(int l = 0; l <= k; ++l) 
			{
				...
				else if(l > 0) 
				{
					...
					if(blockstate.canSurvive(worldgenlevel, blockpos2) && !worldgenlevel.getBlockState(blockpos2.below()).is(OceanicBlocks.GIANT_KELP.get()))
					{
